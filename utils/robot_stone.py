...
class Robot():
    ...
    def __init__(self, tcp_host_ip):
        ...
        self.workspace_limits = np.array([[-0.01, 0.12], [0.63, 0.76], [0.02, 0.08]])
    ...
    def exe_scoop(self, pos, rot_z, ini_aperture, theta = 60*pi/180):  # rot_z rad   aperture distance
        ...
        self.rob.translate((pos[0]-eefPose[0],pos[1]-eefPose[1],0), acc=0.5, vel=0.8)
    ...
    def collision_check_scooping(self, pos, rot_z, ini_aperture, theta = 60*pi/180):
        if Point([pos[0], pos[1]]).within(Point([self.bowl_position[0],self.bowl_position[1]]).buffer(0.06))==False:
            return True
        if Point([pos[0]-ini_aperture*sin(rot_z), pos[1]-ini_aperture*cos(rot_z)]).within(Point([self.bowl_position[0],self.bowl_position[1]]).buffer(0.055))==False:
            return True
        ...
        fingerOrientation_raw = R.from_dcm([[0, 1, 0], [1, 0, 0], [0, 0, -1]]) * R.from_euler('z', rot_z, degrees=False) * R.from_euler('y', pi/2-theta, degrees=False)
        ...
        for finger_thumb_distance in np.arange(ini_aperture+self.l2l+self.l2r, ini_aperture-0.0001+self.l2l+self.l2r, -ini_aperture/5):
            ...
            self.set_joint_positions_collision_check(self.thumb_collision_check, self.THUMB_JOINT_INDICES, start_conf_thumb)
